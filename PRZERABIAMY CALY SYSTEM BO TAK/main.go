...
var MathExpression = EmptyEquation
...
func updateText() {
	...
	// switch { //Eastereggs and help!!

	// }
	expressionText.AutoLineBreak(window.Bounds().Max.X - 30)
	//Check if there are too many characters
	str := strings.SplitAfter(expressionText.Text, "\n")
	if len(str) > 2 {
		expressionText.Text = str[0] + str[1]
		MathExpression = ToEquation(strings.Trim(str[0]+str[1], "\n"))
		MathExpression.Fix()
	}
}

func Calculate() {
	mExpression := MathExpression.ToString()
	...
	if !isInt(string(strMathExpression[0])) && strMathExpression != "-" {
		strMathExpression = strMathExpression[1:]
	}
	MathExpression = ToEquation(strMathExpression)
	index = 0
	updateText()
}
